Cuboidcoin Core version 0.13.2 is now available from:

  <https://download.cuboidcoin.org/cuboidcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/cuboidcoin-project/cuboidcoin/issues>
...
updates will be released anymore. Without security updates, using a cuboidcoin
...
ECDSA signatures inside Cuboidcoin transactions now use validation using
...
Previous versions of Cuboidcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Cuboidcoin Core was to raise the default minimum
relay fee.

Cuboidcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Cuboidcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running cuboidcoind without having to do any manual configuration.
...
Cuboidcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Cuboidcoin Core will relay transactions with
...
entirely in Cuboidcoin Core 0.13, and it is at this time undecided whether the
...
Cuboidcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Cuboidcoin Core automatically creates a hidden service to listen on, without
manual configuration. Cuboidcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Cuboidcoin Core is listening, and
...
Cuboidcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Cuboidcoin
Core to use floating fees. By default, Cuboidcoin Core will use floating
fees.
...
At all times, Cuboidcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Cuboidcoin Core will never create transactions paying less than
...
`cuboidcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /cuboidcoinrpc>
        ...
        # optional bypass cuboidcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< cuboidcoinrpc:<password>
    </Location>
...
Restarting cuboidcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in cuboidcoin.conf
...
cuboidcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/cuboidcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Cuboidcoin Core code base has
...
- `cuboidcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `cuboidcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Cuboidcoin scripting system that in combination with
...
Cuboidcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Cuboidcoin Core.
...
The two phases are distinct in the Cuboidcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Cuboidcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Cuboidcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Cuboidcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of cuboidcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of cuboidcoins being spent is exactly
...
- **Increased security for multisig:** Cuboidcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Cuboidcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Cuboidcoin's current ledger (called the UTXO set),
  ...
  note that Cuboidcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Cuboidcoin users to individually opt-in to almost any change in the Cuboidcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Cuboidcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Cuboidcoin
client to Cuboidcoin Core 0.13.2 to signal for segwit support. When a
...
Since Cuboidcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `cuboidcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `cuboidcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
